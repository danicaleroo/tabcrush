...
export function taskItemHTML(task: Task, settings: Settings): string {
    ...
    <li class="task-item group${task.completed ? " completed" : ""}" data-task-id="${task.id}" role="listitem">
      <button class="task-toggle"
        ...
        <span class="${task.completed ? "completed" : ""}" aria-label="Task: ${sanitizedText}">${sanitizedText}</span>
      </button>
      <div class="task-meta">
        <span class="task-date" aria-label="Created ${dateStr}">${dateStr}</span>
        <button class="task-delete"
          aria-label="Delete task: ${sanitizedText}"
          role="button">
...
export function emptyStateHTML(): string {
	return '<div class="empty-state hidden-state">No completed tasks.</div>';
}
	...
	return `
      ...
      <div class="flex justify-between items-end gap-4 flex-wrap">
        <div class="flex items-center gap-2">
          <a href="https://github.com/tabcrush" target="_blank" rel="noopener noreferrer" class="text-500" aria-label="Visit GitHub repository">
        ...
        <button type="button" class="close-button focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2">Close</button>
      </div>
  `;
}
